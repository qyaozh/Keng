...
`move_O()` mean-centers x by default. `move_O()` could move the origin of x forward by `coord` distance is `coord` is specified. `move_O()` could be used to pick points in simple-slope analysis for moderation models.

`std()` transform x into z-scores by default. `std()` could standardize the *M* and *SD* of x into other values than 0 and 1.
...
`compare_lm()` compares `lm()`'s fitted outputs using *PRE* and *R*^2^.
