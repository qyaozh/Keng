...
depress_factor$class100 <- factor(depress_factor$class100, labels = c(3,5,9,12))
depress_factor$gender100 <- factor(depress_factor$gender100, labels = c(0,1))

anova.fit <- lm(depr1 ~ gender100 + class100, depress_factor)
...
depress$class3 <- ifelse(depress$class100 == 1, 1, 0)
# class5 indicates whether the class is class5 
depress$class5 <- ifelse(depress$class100 == 2, 1, 0)
# class9 indicates whether the class is class9 
depress$class9 <- ifelse(depress$class100 == 3, 1, 0)
```
...
fitA <- lm(depr1 ~ class3 + class5 + class9 + gender100, depress)
...
```{r}
fitC <- lm(depr1 ~ gender100, depress)
fitA <- lm(depr1 ~ class3 + class5 + class9 + gender100, depress)
print(compare_lm(fitC, fitA), digits = 3)
```
