...
#'
#' @return A data.frame including *SSEC*, *SSEA*, *PRE*, *F*, *df1*, *df2*, *p*, and *PRE_adjusted*.
#' @export
#'
#' @examples
#' x <- rnorm(193)
...
test_PRE <- function(fitC=NULL, fitA=NULL, n=NULL, PC=NULL, PA=NULL, SSEC=NULL, SSEA=NULL) {
  ...
  PRE_adjusted <- 1 - (1 - PRE)*((n - PC)/(n - PA))
  # R Square

  # Return
  out <- as.data.frame(matrix(
    c(SSEC, NA, NA, NA, NA, NA, NA,
      SSEA, PRE, F, df1, df2, p, PRE_adjusted),
    nrow = 2, ncol = 7, byrow = TRUE,
    dimnames = list(
      c("Model C","Model A"),
      c("SSE", "PRE", "F", "df1", "df2","p","PRE_adjusted")
    )))
  return(out)
}
